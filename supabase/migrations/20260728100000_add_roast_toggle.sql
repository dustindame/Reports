-- Lets the commissioner turn the Draft Recap's roast section on or off per
-- league (some groups want the savage commentary, some don't).

alter table public.draft_config
  add column if not exists roast_enabled boolean not null default true;

grant select (roast_enabled) on public.draft_config to anon;

drop function if exists public.create_league(text, text, integer, integer, jsonb, jsonb, text, boolean, boolean, boolean, boolean, boolean, boolean, boolean, integer, jsonb);
...
begin
  insert into public.draft_config (
    ...
    nice_enabled, shots_count, shot_pick_numbers, roast_enabled
  )
  values (
    ...
    p_nice_enabled, p_shots_count, p_shot_pick_numbers, p_roast_enabled
  )
...
grant execute on function public.create_league(text, text, integer, integer, jsonb, jsonb, text, boolean, boolean, boolean, boolean, boolean, boolean, boolean, integer, jsonb, boolean) to anon;
grant execute on function public.update_league(text, text, integer, integer, jsonb, jsonb, boolean, text, boolean, boolean, boolean, boolean, boolean, boolean, boolean, integer, jsonb, boolean) to anon;
